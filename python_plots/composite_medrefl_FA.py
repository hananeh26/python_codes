...
# This code is plotting composite reflectivity at genesis time for FA.
...
for l in range(len(not_genesis)):
    if re.search(regex, not_genesis[l]):
        miss_cases.append(not_genesis[l])
        Misses += 1
    else:
        Correct_negatives +=1
...
for at in range(len(False_alarm_cases)):
    all_date.append(str(False_alarm_cases[at].split('.')[1][:10])) #2019082306
    all_fhr.append(str(False_alarm_time[at])) #48
    all_sid.append(str(False_alarm_cases[at].split('.')[0][-3:-1]))
...
#    print(len(min_lon))
    ...
    RF.append(grbs.variables['refd'][28,minlat[:],minlon[:]])
...
lon_c=np.median(lon_c)-360
lat_c=np.median(lat_c)

z=np.nanmedian(RF,axis=0)
...
dbz_map, dbz_norm = from_levels_and_colors(dbz_levels, dbz_rgb,\
                                           extend="max")
contour_c=plt.pcolormesh(x, y, z,\
                                 cmap=dbz_map,\
                                 norm=dbz_norm)
...
font.set_name('Times')
plt.title("Median Refl for FA at 300mb at Genesis",fontsize='small',fontproperties=font )
...
plt.savefig(picture_dir+"Med_Reflectivity_FA_300mb"+'.png')
plt.close()
